...
class KomorebiClient:
    ...
    def ensure_workspaces(self, m_idx: int, count: int, wait: bool = False) -> None:
        args = [self._komorebic_path, "ensure-workspaces", str(m_idx), str(count)]
        if wait:
            try:
                subprocess.run(args, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, shell=True)
            except (subprocess.SubprocessError, FileNotFoundError):
                logging.exception("Failed to ensure komorebi workspaces")
        else:
            try:
                subprocess.Popen(args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
            except (subprocess.SubprocessError, FileNotFoundError):
                logging.exception("Failed to ensure komorebi workspaces (spawn)")
